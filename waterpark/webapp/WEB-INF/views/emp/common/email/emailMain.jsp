		...
		<div class="row">
			<div class="col-lg-12">
				...
				<ol class="breadcrumb">
					...
					<li><i class="fa fa-laptop"></i>받은 쪽지</li>
				</ol>
			</div>
		</div>
		<div class="row">
			<div class="col-lg-12 col-md-15">
				<div class="panel panel-default">
					<div class="panel-heading">
						<h2>
							<i class="fa fa-flag-o red"></i><strong>쪽지 목록</strong>
						</h2>
					</div>
							...
							<div class="tableBox">
								...
								<thead>
									<tr height="1">
										...
										<th>번호</th>
										<th>보낸이</th>
										<th class="centering">제목</th>
										<th class="centering">날짜</th>
										<th class="centering">읽음 여부</th>
									</tr> 
								</thead>
								<tbody id="listBody">
									<c:choose>
										<c:when test="${not empty pagingVO.dataList }">
											<c:forEach items="${pagingVO.dataList }" var="email">
												<tr class="email-item">
													...
													<td>${email.rnum }</td>
													<c:url value="/emp/emailView" var="viewURL">
														<c:param name="what" value="${email.email_no}" />
														<c:param name="receiver_id" value="${email.receiver_id}" />
													</c:url>
...
//
	...
	let searchForm = $("#searchForm").ajaxForm({
		...
		, beforeSubmit:function(){
			...
			let trTags = [];
			if(resp.dataList){
				let viewURLPtrn = "${WCP}/emp/emailView?what=%d";
				$(resp.dataList).each(function(idx,email){
					...
					let tr = $("<tr>").append(
							...
							,$("<td>").html(email.rnum)
							,$("<td>").html(email.sender_name)
							,$("<td>").html(aTag)
							,$("<td class='centering'>").html(email.send_date)
							,$("<td class='centering'>").html('<c:choose><c:when test="${not empty email.receive_date}">${email.receive_date}</c:when><c:otherwise>X</c:otherwise></c:choose>')
	...
	$(".goRead").on("click",function(){
		...
		Swal.fire({
			  title: '읽음 처리할까요?',
			  text: "선택한 쪽지들을 읽음 처리합니다.",
			  icon: 'info',
			  showCancelButton: true,
			  confirmButtonColor: '#3085d6',
			  cancelButtonColor: '#d33',
			  confirmButtonText: '읽기'
			}).then((result) => {
				   if (result.isConfirmed) {
					   $("input[name=chk]:checked").each(function() { 
						...
						$.ajax({
							...
							success : function(resp){
								if(resp.result == "OK"){
									intercept("읽기 완료","선택한 이메일을 읽기 처리했습니다.", 'success',false);
	...
	$(".goDelete").on("click",function(){
		...
		Swal.fire({
			...
			$.ajax({
				...
				success : function(resp){
					if(resp.result == "OK"){
						intercept("삭제 완료","선택한 이메일을 삭제 처리했습니다.", 'success',false);
