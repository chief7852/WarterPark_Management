		...
		<div class="row">
			<div class="col-lg-12">
				...
				<ol class="breadcrumb">
					...
					<li><i class="fa fa-laptop"></i>휴지통</li>
				</ol>
			</div>
		</div>
		<div class="row">
			...
			if(resp.dataList){
				...
				$(resp.dataList).each(function(idx,email){
					...
					let tr = $("<tr>").append(
							...
							,$("<td>").html(aTag)
							,$("<td>").html(email.send_date)
							,$("<td>").html('<c:choose><c:when test="${not empty email.receive_date}"><td>${email.receive_date}</td></c:when><c:otherwise><td>X</td></c:otherwise></c:choose>')
